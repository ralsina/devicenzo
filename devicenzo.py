...
from PySide2 import QtWidgets, QtGui, QtCore, QtWebEngineWidgets, QtNetwork
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        # self.tabs = QtWidgets.QTabWidget(self, tabsClosable=True, movable=True, currentChanged=self.currentTabChanged, elideMode=QtCore.Qt.ElideRight, tabCloseRequested=lambda idx: self.tabs.widget(idx).deleteLater())
        self.tabs = QtWidgets.QTabWidget(self, tabsClosable=True, movable=True, elideMode=QtCore.Qt.ElideRight)
        self.setCentralWidget(self.tabs)
        self.bars = {}
        self.star = QtWidgets.QAction(QtGui.QIcon.fromTheme("emblem-favorite"), "Bookmark", self, checkable=True, triggered=self.bookmarkPage, shortcut="Ctrl+d")
        self.tabs.setCornerWidget(QtWidgets.QToolButton(self, text="New Tab", icon=QtGui.QIcon.fromTheme("document-new"), clicked=lambda: self.addTab().url.setFocus(), shortcut="Ctrl+t"))
        # self.addAction(QtWidgets.QAction("Full Screen", self, checkable=True, toggled=lambda v: self.showFullScreen() if v else self.showNormal(), shortcut="F11"))
        self.addAction(QtWidgets.QAction("Full Screen", self, checkable=True, shortcut="F11"))
        ...
        QtNetwork.QNetworkProxy.setApplicationProxy(QtNetwork.QNetworkProxy(QtNetwork.QNetworkProxy.HttpProxy if proxy_url.scheme().startswith('http') else QtNetwork.QNetworkProxy.Socks5Proxy, proxy_url.host(), proxy_url.port(), proxy_url.userName(), proxy_url.password())) if 'http_proxy' in os.environ else None

        [self.addTab(QtCore.QUrl(u)) for u in self.get("tabs", [])]

    def fetch(self, reply):
        destination = QtWidgets.QFileDialog.getSaveFileName(self, "Save File", os.path.expanduser(os.path.join('~', reply.url().path().split('/')[-1])))
        if destination:
            bar = QtWidgets.QProgressBar(format='%p% - ' + os.path.basename(destination))
            cancel = QtWidgets.QToolButton(bar, icon=QtGui.fromTheme("process-stop"), clicked=reply.abort)
    ...
    def finished(self):
        ...
        redirURL = reply.attribute(QtNetwork.QNetworkRequest.RedirectionTargetAttribute).toString()
        del self.bars[url]
        bar.deleteLater()
        cancel.deleteLater()
    ...
    progress = lambda self, received, total: self.bars[self.sender().url().toString()][0].setValue(100. * received / total)

    def closeEvent(self, ev):
        self.put("history", self.history)
        self.put("cookiejar", [str(c.toRawForm()) for c in self.cookies.allCookies()])
        self.put("tabs", [self.tabs.widget(i).url.text() for i in range(self.tabs.count())])
    ...
    def bookmarkPage(self, v=None):
        if v and v is not None:
            self.bookmarks[self.tabs.currentWidget().url.text()] = self.tabs.currentWidget().wb.title()
        elif v is not None:
            del (self.bookmarks[self.tabs.currentWidget().url.text()])
        self.star.setMenu(QtWidgets.QMenu())
        [self.star.menu().addAction(QtWidgets.QAction(title, self, triggered=lambda u=QtCore.QUrl(url): self.tabs.currentWidget().load(u))) for url, title in self.bookmarks.items()]
...
class Tab(QtWidgets.QWidget):
    def __init__(self, url, container):
        ...
        self.wb.urlChanged.connect(lambda u: container.star.setChecked(u.toString() in container.bookmarks) if self.amCurrent() else None)
        ...
        self.showSearch = QtWidgets.QShortcut("Ctrl+F", self, activated=lambda: self.search.show() or self.search.setFocus())
        self.hideSearch = QtWidgets.QShortcut("Esc", self, activated=lambda: (self.search.hide(), self.setFocus()))
        ...
        self.do_close = QtWidgets.QShortcut("Ctrl+W", self, activated=lambda: container.tabs.removeTab(container.tabs.indexOf(self)))
        self.do_quit = QtWidgets.QShortcut("Ctrl+q", self, activated=lambda: container.close())
        self.zoomIn = QtWidgets.QShortcut("Ctrl++", self, activated=lambda: self.wb.setZoomFactor(self.wb.zoomFactor() + 0.2))
        self.zoomOut = QtWidgets.QShortcut("Ctrl+-", self, activated=lambda: self.wb.setZoomFactor(self.wb.zoomFactor() - 0.2))
        self.zoomOne = QtWidgets.QShortcut("Ctrl+0", self, activated=lambda: self.wb.setZoomFactor(1))
