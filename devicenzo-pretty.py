...
class MainWindow(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.tabs = QtGui.QTabWidget(self,
            ...
            tabCloseRequested=lambda idx: self.tabs.widget(idx).deleteLater())  # XXX expandir
        ...
        self.addAction(
            QtGui.QAction(
                ...
                self,
                checkable=True,
                toggled=lambda v: self.showFullScreen() if v else self.showNormal(),  # XXX EXPANDIR
                shortcut="F11"))
        ...
        self.cookies.setAllCookies(
            [QtNetwork.QNetworkCookie.parseCookies(c)[0] for c in self.get("cookiejar", [])])
        ...
        for u in self.get("tabs", []):
            self.addTab(QtCore.QUrl(u))

    def fetch(self, reply):
        destination = QtGui.QFileDialog.getSaveFileName(
            self,
            "Save File",
            os.path.expanduser(os.path.join('~', unicode(reply.url().path()).split('/')[-1]))) # XXX desenfear
        if destination:
            bar = QtGui.QProgressBar(format='%p% - ' + os.path.basename(unicode(destination)))
            cancel = QtGui.QToolButton(
                bar,
    ...
    def progress(self, received, total):
        self.bars[unicode(self.sender().url().toString())][0].setValue(100. * received / total)
