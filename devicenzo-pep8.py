...
class Tab(QtWebKit.QWebView):
    def __init__(self, url, container):
        ...
        QtWebKit.QWebView.__init__(self,
            ...
            loadFinished=self.pbar.hide, loadStarted=lambda: self.pbar.show()
                if self.amCurrent() else None,
        ...
        self.newtab = QtGui.QAction(QtGui.QIcon.fromTheme("document-new"),
            "New Tab", self, triggered=self.createWindow)
        self.tb.addAction(self.newtab)

        self.urlChanged.connect(lambda u: self.url.setText(u.toString()))
        self.urlChanged.connect(lambda: self.url.setCompleter(
            QtGui.QCompleter(QtCore.QStringList([
                QtCore.QString(i.url().toString())
                for i in self.history().items()]),
            caseSensitivity=QtCore.Qt.CaseInsensitive)))
        self.urlChanged.connect(lambda u:
            self.star.setChecked(unicode(u.toString()) in self.bookmarks))
        ...
        self.zoomIn = QtGui.QShortcut("Ctrl++", self,
            activated=lambda: self.setZoomFactor(self.zoomFactor() + 0.2))
        self.zoomOut = QtGui.QShortcut("Ctrl+-", self,
            activated=lambda: self.setZoomFactor(self.zoomFactor() - 0.2))
        self.zoomOne = QtGui.QShortcut("Ctrl+=", self,
            activated=lambda: self.setZoomFactor(1))
    ...
    def get(self, key, default=None):
        "Get the object under key in persistent storage, or the default value"
        v = settings.value(key)
        return json.loads(unicode(v.toString())) if v.isValid() else default

    def bookmarkPage(self, v=None):
        if v and v is not None:
            self.bookmarks[unicode(self.url.text())] = \
                unicode(self.windowTitle())
        elif v is not None:
        ...
        [self.star.menu().addAction(QtGui.QAction(title, self,
            activated=lambda u=QtCore.QUrl(url):
                self.load(u))) for url, title in self.bookmarks.items()]
