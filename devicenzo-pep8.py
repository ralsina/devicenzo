...
class MainWindow(QtGui.QMainWindow):
    ...
    def fetch(self, reply):
        ...
        if destination:
            bar = QtGui.QProgressBar(
                format='%p% - ' + os.path.basename(unicode(destination)))
            self.statusBar().addPermanentWidget(bar)
            reply.downloadProgress.connect(self.progress)
            reply.finished.connect(self.finished)
            self.bars[unicode(reply.url().toString())] =\
                [bar, reply, unicode(destination)]
    ...
    def closeEvent(self, ev):
        ...
        self.put("cookiejar", [str(c.toRawForm())
            for c in self.cookies.allCookies()])
        self.put("tabs", [unicode(self.tabs.widget(i).url.text())
    ...
    def addToHistory(self, url):
        ...
        self.completer.setModel(QtGui.QStringListModel(list(set(
            self.bookmarks.keys() + self.history))))
    ...
    def __init__(self, url, container):
        ...
        QtWebKit.QWebView.__init__(self,
            loadProgress=lambda v:
                (self.pbar.show(), self.pbar.setValue(v)) if self.amCurrent() \
                else None,
        ...
        self.urlChanged.connect(lambda u:
            self.url.setText(u.toString()))
        self.urlChanged.connect(lambda u:
            container.addToHistory(unicode(u.toString())))
        self.urlChanged.connect(lambda u:
            container.star.setChecked(
